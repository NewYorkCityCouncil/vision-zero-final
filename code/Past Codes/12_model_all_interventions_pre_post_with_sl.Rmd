...
```{r}
# uploading
sites_ever_treated_sl <- read_csv('../data/output/intersection_intervention_table_with_speed_limit_only_2015-2022.csv', show_col_types = FALSE)
# meant to handle large datasets
sites_ever_treated_sl <- as.data.table(sites_ever_treated_sl)
```

```{r}
# Matrix for interventions that never received speed limit change

# --- Step 1: Explicitly define the list of the main interventions ---
main_interventions <- c(
...
# --- Step 2: Identify the intersection_ids that NEVER had the speed limit change ---
ids_without_sl <- sites_ever_treated_sl %>%
  group_by(intersection_id) %>%
  summarise(max_sl = max(speed_limit_post), .groups = 'drop') %>%
  filter(max_sl == 0) %>%
  pull(intersection_id)

cat(sprintf("Found %d unique intersections that never received the speed limit change.", length(ids_without_sl)))

# --- Step 3: Create the summary matrix of treatments for this cohort ---
if (length(ids_without_sl) > 0) {
  treatment_summary_matrix <- sites_ever_treated_sl %>%
    # Filter to only the intersections of interest
    filter(intersection_id %in% ids_without_sl) %>%
    # Group by intersection to analyze each one's history
    group_by(intersection_id) %>%
    # CRITICAL CHANGE: Use the explicit list of intervention columns.
    # The `all_of()` wrapper ensures that if a column name is mistyped,
    # R will give an informative error.
    summarise(across(all_of(main_interventions), ~max(.x, na.rm = TRUE)), .groups = 'drop') %>%
    # The rest of the pipeline will now work correctly
    select(-intersection_id) %>%
    colSums() %>%
    enframe(name = "Intervention_Type", value = "Count_of_Intersections") %>%
    arrange(desc(Count_of_Intersections))

  # --- Step 4: Display the results in a clean, informative table ---
  cat("\n\n--- Summary of Treatments for Intersections Without Speed Limit Change ---\n")
  
  knitr::kable(
    treatment_summary_matrix,
    col.names = c("Intervention Type", "Number of Intersections Receiving It"),
    caption = "Frequency of Interventions Among Intersections Never Receiving a Speed Limit Change"
  )
  
  # --- Step 5: Create the bar chart ---
  treatment_plot <- treatment_summary_matrix %>%
    mutate(Intervention_Type = fct_reorder(Intervention_Type, Count_of_Intersections)) %>%
    ggplot(aes(x = Intervention_Type, y = Count_of_Intersections, fill = Intervention_Type)) +
    geom_col(show.legend = FALSE) +
    coord_flip() +
    labs(
      title = "Which Treatments Did Intersections Without Speed Limit Change Receive?",
      subtitle = "Counts of unique intersections receiving each intervention type",
      x = "",
      y = "Number of Intersections"
    ) +
    theme_minimal(base_size = 12) +
    geom_text(aes(label = Count_of_Intersections), hjust = -0.2, size = 3.5) +
    scale_y_continuous(expand = expansion(mult = c(0, 0.1))) 

  print(treatment_plot)

} else {
  cat("\n\nNo intersections were found that did not have the speed limit change.")
}
```
...
sites_ever_treated_sl <- sites_ever_treated_sl %>%
  mutate(
...
poisson_model <- glm(
  pedestrian_death_or_injury ~ 
    ...
    (1 | intersection_id),
  family = "poisson", 
  data = sites_ever_treated_sl
)

# calculate dispersion statistic
# dispersion stat over 1.4 indicates needs for negative binomial
...
```{r}
# pedestrians
# negative binomial regression (dispersion stat: 1.4)
...
# best for KS test ~ scaled_summonses + covid_2020 + speed_limit_post
...
model_dispformula2 <- glmmTMB(
  pedestrian_death_or_injury ~ 
    ...
    source +
    (1 | boro_name/ntaname/intersection_id),
  dispformula = ~ scaled_summonses + covid_2020 + speed_limit_post,
  family = nbinom1,
  data = sites_ever_treated_sl
) 
...
```{r}
# cluster bootstrap robust standard errors (takes 19 hours to run with R = 1000)
...
cluster_bootstrap_glmmTMB(model = model_spline, data = sites_ever_treated_sl, cluster_var = "intersection_id", R = 1)
...
# 1. Define the list of "other" intervention columns
other_interventions_cols <- c(
...
# 2. Re-identify the 'other_interventions' cohort using the strict "clean history" logic.

# Step A: Find the start year for EACH intervention type within EACH intersection.
intervention_start_dates <- all_sites %>%
  select(intersection_id, year, all_of(other_interventions_cols)) %>%
  pivot_longer(
    cols = all_of(other_interventions_cols),
...
# Step B: For each intersection, check if ALL of its intervention start years are within the window.
intersection_history_summary <- intervention_start_dates %>%
  group_by(intersection_id) %>%
  summarise(
    is_clean_history = all(between(start_year, 2015, 2021))
  )

# Step C: The final set of IDs are only those whose history is "clean".
ids_kept_other_interventions <- intersection_history_summary %>%
  filter(is_clean_history == TRUE) %>%
  pull(intersection_id)

# 3. Create the 'source' column for the ENTIRE dataset using your default logic.
all_sites <- all_sites %>%
  mutate(
    source = case_when(
      intersection_id %in% ids_kept_other_interventions ~ "other_interventions",
      TRUE ~ "speed_limit_only"
    ),
    source = factor(source, levels = c("other_interventions", "speed_limit_only"))
  )

# This verification should now pass perfectly.

cat("--- EXPLAINING THE ROW COUNT DIFFERENCE ---\n")

# Isolate all the rows that we just labeled as 'other_interventions' in the full dataset
other_interventions_in_all_sites <- all_sites %>%
  filter(source == "other_interventions")

# Count how many of these rows fall INSIDE the original analytic window (2013-2023)
rows_inside_window <- other_interventions_in_all_sites %>%
  filter(between(year, 2013, 2023)) %>%
  nrow()

# Count how many of these rows fall OUTSIDE the original analytic window
rows_outside_window <- other_interventions_in_all_sites %>%
  filter(!between(year, 2013, 2023)) %>%
  nrow()

cat("Total rows labeled 'other_interventions' in all_sites:", nrow(other_interventions_in_all_sites), "\n")
cat("----------------------------------------------------------\n")
cat("Rows INSIDE the 2013-2023 window:", rows_inside_window, "(This MUST match your original count)\n")
cat("Rows OUTSIDE the 2013-2023 window:", rows_outside_window, "(This explains the difference)\n")
cat("----------------------------------------------------------\n")

if (rows_inside_window == 105182) {
    cat("✅✅✅ PERFECT MATCH: The number of 'other_interventions' rows within the 2013-2023 period is now exactly 105,182, perfectly matching your training data.\n\n")
} else {
    cat("❌❌❌ MISMATCH: The count is still incorrect. Expected 105182 but got", rows_inside_window, "\n\n")
}
```
...
df_cf$street_improvement_corridors_post <- 0
...
total_casualties_prevented_sig <- sum(all_sites$casualties_prevented[all_sites$turn_traffic_calming_post == 1]) + 
  sum(all_sites$casualties_prevented[all_sites$speed_humps_post == 1]) +  sum(all_sites$casualties_prevented[all_sites$street_improvement_corridors_post == 1]) + sum(all_sites$casualties_prevented[all_sites$slow_zones_post == 1]) 
sum(all_sites$casualties_prevented[all_sites$speed_limit_post == 1])
