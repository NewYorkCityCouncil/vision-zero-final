...
```{r}
# get unique intersections for each treatment

sites_ever_treated %>%
  pivot_longer(
    cols = c(leading_pedestrian_interval_post,
  ...
  filter(treated == 1) %>%
  # Group by the intervention type
  group_by(intervention) %>%
  # Calculate both summaries at once
  summarise(
    # This counts the number of unique intersection_ids for each intervention
    n_unique_intersections = n_distinct(intersection_id), 
    
    # This counts the total number of rows for each intervention
    n_total_rows = n()
  ) %>%
...
# best for KS test ~ scaled_pop_density + scaled_traffic_volume + scaled_summonses + covid_2020
...
model_dispformula2 <- glmmTMB(
  pedestrian_death_or_injury ~ 
    ...
    scaled_time +       
    (1 | boro_name/ntaname/intersection_id),
  dispformula = ~ scaled_pop_density + scaled_traffic_volume + scaled_summonses + covid_2020,
  family = nbinom1,
  data = sites_ever_treated
) 
